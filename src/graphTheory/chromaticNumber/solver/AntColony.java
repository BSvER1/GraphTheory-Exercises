...
public class AntColony {
	...
	long[][] vertexAnts; // ants currently in the vertex
							// vertexAnts[vertex][colour] = number of ants at
							// vertex with colour.
	long[][] nextIterationAnts; // ants coming to the vertex next iteration
	int[] currentVertexColour; // currently agreed upon vertex colouring for
								// [vertex]

	double commonRate = 1.0; // rate of ants that go to (the first found) edge
								// of the same colour
	long refillAmount;
	int antResult;
	int currentShortest;
	 ...
	 *            maximum number of iterations to perform in any single
	 *            simulation
	 * @param numColours
	 *            the number of colours to try and match.
	 * @return the number of colours successfully matched before reaching the
	 *         iteration limit.
	 */
	public int solve(Graph g, long iterationLimit, int numColours) {
		...
		setInitial(g, numColours); // TODO dont do this, just modify data
									// instead.
		populateInitial(g, numColours);
		resetNext(g);

		while (simResult) {
			// Driver.trace(this.getClass(), "setting initial conditions for
			// ants");

			for (int i = 0; i < g.getNumVertices(); i++) {
				determineLargestColour(i, iterationColours);
			}
			// Driver.trace(this.getClass(), "finished setting initial
			// conditions");

			simResult = runSimulation(g, iterationLimit, iterationColours);
			if (simResult) {
				Driver.trace(System.currentTimeMillis() - timeStart, "found graph colouring "
						+ "with " + iterationColours+ " colours in " + currentIteration + ""
								+ " cycles");
				iterationColours--;

				// shorten arrays, making ants move around a little more. dont
				// discard progress. do this by removing the smallest colouring
				...
				Driver.trace("failed to find graph colouring with " + iterationColours + " colours");
			}
		}
	...
	/**
	 * Performs simulation with given amounts of ants, with a limit on the
	 * simulation time.
	 * 
	 * @param g
	 *            Graph to perform sim on.
	 * @param iterationLimit
	 *            maximum number of cycles allowed in any simulation.
	 * @param numColours
	 *            number of colours to try and match.
	 * @return Returns true if a valid colouring is encountered. otherwise false
	 *         (reached computation limit)
	 */
	private boolean runSimulation(Graph g, long iterationLimit, int numColours) {
		...
		while (currentIteration < iterationLimit && !isValidColouring(g)) {
			currentIteration++;
			// Driver.trace(this.getClass(), "Starting iteration "+
			// currentIteration);
			for (int i = 0; i < g.getNumVertices(); i++) {
	...
	private void breakConflicts(Graph g) {
		for (int i = 0; i < vertexAnts.length; i++) { // for all vertices
			for (int j = i + 1; j < vertexAnts.length; j++) { // for all other
																// vertices
				if (g.isEdge(i, j) && currentVertexColour[i] == currentVertexColour[j]) {
					// Driver.trace(this.getClass(), "found a conflict.
					// fixing");
	 ...
	 * 
	 * @param g
	 *            Graph object with vertices to iterate over.
	 */
	 ...
	 * 
	 * @param g
	 *            Graph object that vertices are part of.
	 * @param vertex
	 *            Specified vertex to fill with ants.
	 * @param numColours
	 *            the number of colours to spread ants across
	 */
	private void refillVertex(Graph g, int vertex, int numColours) {
		// Driver.trace(this.getClass(), "found that vertex "+ vertex+ " has no
		// ants. filling it back up");
	...
	/**
	 * shift all ants currently in the next iteration phase to the current phase
	 * by adding them to the current pool. resets the next iteration ant pool
	 * ready to start again.
	 */
	private void promoteNewToCurrent() {
		// Driver.trace(this.getClass(), "promoting ants waiting to come into
		// vertices to counts");
		for (int i = 0; i < vertexAnts.length; i++) {
	...
	/**
	 * Determine the current optimal colouring of a vertex. split ties by the
	 * lower vertex number. This method updates the global variable
	 * {@code currentVertexColour} at {@code vertex} at the same time.
	 * 
	 * @param vertex
	 *            The vertex to consider when counting the ants
	 * @return the colouring the vertex currently has as voted on by the ants
	 *         currently there.
	 */
	private int determineLargestColour(int vertex, int numColours) {
		...
		// Driver.trace(this.getClass(), "found largest colour of vertex
		// "+vertex+" to be "+currentVertexColour[vertex]);
		return currentVertexColour[vertex];
	}

	/**
	 * Send ants that don't suit the current vertex colouring to the adjacent
	 * vertices.<br>
	 * 
	 ...
	 * @param vertex
	 *            Vertex to send ants away from.
	 */
	private void deployForeigners(Graph g, int vertex) {
		// Driver.trace(this.getClass(), "beginning deployment of ants from
		// vertex "+ vertex);
		...
		// Driver.trace(this.getClass(), "finished deploying ants from vertex
		// "+vertex);
	}
	 ...
	 *            Graph containing vertices and edges.
	 * @param src
	 *            number of the source vertex
	 * @param col
	 *            id of the colour to match
	...
	private int findAdjacentWithColour(Graph g, int src, int col) {
		for (int i = 0; i < g.getNumVertices(); i++) {
			if (g.isEdge(src, i)) { // is edge adjacent?
				if (currentVertexColour[i] == col) { // is adjacent vertex the
														// same colour.
					// Driver.trace(this.getClass(), "found that vertex "+i+"
					// has colour matching vertex "+src+" ("+col+")");
					return i; // return this vertex
				}
			}
		}
		// Driver.trace(this.getClass(), "could not find a vertex adjacent to
		// "+src+ " with colour "+ col);
	...
	private boolean isValidColouring(Graph g) {
		for (int start = 0; start < g.getNumVertices(); start++) {
			for (int end = 0; end < g.getNumVertices(); end++) {
				if (start != end && g.isEdge(start, end) && currentVertexColour[start] == currentVertexColour[end]) {
					// Driver.trace(this.getClass(), "found that the current
					// colouring is invalid");
					return false;
				}
			}
		}

		// Driver.trace(this.getClass(), "found that the current colouring is
		// valid");
		return true;
	}

	/**
	 * Determine if all the ants have left a vertex. Indicates that it needs a
	 * refill.
	 * 
	 * @param g
